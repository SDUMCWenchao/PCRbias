...
FEATURE_DIR="${EXT}/analysis_results/02_Features"
...
submit_batch () {
  ...
  local SLURM_SCRIPT="${FEATURE_DIR}/run_ext04_agg_${START_TASK_ID}_n${BATCH_N}.slurm"

  cat > "${SLURM_SCRIPT}" <<EOF
#!/usr/bin/env bash
#SBATCH --job-name=extKmerAgg
#SBATCH --cpus-per-task=${CPUS}
#SBATCH --mem=${MEM}
#SBATCH --array=0-${END_INDEX}%${CONC}
#SBATCH --output=${LOG_DIR}/extKmerAgg_%A_%a.out
#SBATCH --error=${LOG_DIR}/extKmerAgg_%A_%a.err
set -euo pipefail
source ~/.bashrc
...
python "${SCRIPTS}/02b1_agg_kmers_chunk.py" --in_kmer "\${IN_KMER}" --out_agg "\${OUT_AGG}"
EOF
...
echo "[DONE] submitted external kmer agg"
