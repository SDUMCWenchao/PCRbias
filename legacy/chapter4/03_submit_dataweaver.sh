...
OUT_DIR="${PROJECT_DIR}/analysis_results/03_DataWeaver"
...
submit_batch () {
  ...
  local SLURM_SCRIPT="${OUT_DIR}/run_03_weaver_${START_TASK_ID}_n${BATCH_N}.slurm"

  cat > "${SLURM_SCRIPT}" <<EOF
#!/usr/bin/env bash
#SBATCH --job-name=weaver3
#SBATCH --cpus-per-task=${CPUS}
#SBATCH --mem=${MEM}
#SBATCH --array=0-${END_INDEX}%${CONC}
#SBATCH --output=${LOG_DIR}/weaver3_%A_%a.out
#SBATCH --error=${LOG_DIR}/weaver3_%A_%a.err
...
python "${SCRIPTS_DIR}/03b_dataweaver_chunk.py" --project_dir "${PROJECT_DIR}" --chunk_name "\${CHUNK_NAME}"
EOF
...
echo "[DONE] Submitted Step 3B (data weaver)."
