...
def find_xgb_model_file(model_dir: Path):
    cands = ["model.json","xgb.json","booster.json","model.ubj","model.bin","xgb.model"]
    for c in cands:
        p = model_dir / c
        if p.exists() and p.stat().st_size > 0:
            return p
    for p in sorted(model_dir.glob("*.ubj")) + sorted(model_dir.glob("*.bin")) + sorted(model_dir.glob("*.json")):
        if p.stat().st_size > 0:
            return p
    return None
...
def main():
    ...
    mf = find_xgb_model_file(md)
    if mf is None:
        raise SystemExit(f"[ERROR] cannot find XGB model file in {md}")
    booster = xgb.Booster()
    booster.load_model(str(mf))
    ...
    dmat = xgb.DMatrix(Xex, feature_names=feat)
    contrib = booster.predict(dmat, pred_contribs=True)  # (m, p+1) last=bias
    ...
    info = {"method":"XGB pred_contribs(TreeSHAP)","model_file":str(mf),"explain_n":int(len(e_idx)),"seed":args.seed}
    (out / f"shap_info_{args.split}.json").write_text(json.dumps(info, indent=2) + "\n", encoding="utf-8")

    print(f"[DONE] XGB SHAP(v2) -> {out}")
